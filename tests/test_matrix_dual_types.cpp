...
template <typename TupleType> class MatrixDualTypesTest : public ::testing::Test {
protected:
  using T = typename std::tuple_element<0, TupleType>::type;
  using U = typename std::tuple_element<1, TupleType>::type;

  supp_math::Matrix<T> first_test_matrix = supp_math::Matrix<T>({
      {3,  5, -1},
      {0, -5, -4},
      {8,  1,  3}
  });

  supp_math::Matrix<U> second_test_matrix = supp_math::Matrix<U>({
      {  7, -9, 1},
      { 11, -7, 2},
      {-12,  6, 3}
  });

  supp_math::Matrix<std::common_type_t<T, U>> expected_addition = supp_math::Matrix<std::common_type_t<T, U>>({
      {10,  -4,  0},
      {11, -12, -2},
      {-4,   7,  6}
  });

  supp_math::Matrix<std::common_type_t<T, U>> expected_subtraction = supp_math::Matrix<std::common_type_t<T, U>>({
      { -4, 14, -2},
      {-11,  2, -6},
      { 20, -5,  0}
  });

  supp_math::Matrix<std::common_type_t<T, U>> expected_multiplication = supp_math::Matrix<std::common_type_t<T, U>>({
      {88, -68,  10},
      {-7,  11, -22},
      {31, -61,  19}
  });

  supp_math::Matrix<std::common_type_t<T, U>> expected_element_wise_multi =
